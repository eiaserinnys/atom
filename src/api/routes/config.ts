...
const defaultMigrationRouteDeps: MigrationRouteDeps = {
  getDb,
  getSqlitePath: () => process.env['SQLITE_PATH'] ?? path.join(process.cwd(), 'atom.db'),
  existsSync: fs.existsSync,
  openSqliteReadonly: (sqlitePath) => new Database(sqlitePath, { readonly: true }) as SqliteMigrationDatabase,
  renameSync: fs.renameSync,
};

export function createMigrateToPgHandler(
  deps: MigrationRouteDeps = defaultMigrationRouteDeps,
...
export const configRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/config/migrate-to-pg', createMigrateToPgHandler());
